...
`corridas/corrida_2026-09-03_GS_BBH/corrida.json` → sección `execution_log`
...
`corridas/corrida_2026-09-03_GS_BBH/corrida.json` → sección `human_review`
...
## D-22 — Productivizar la capa de inferencia y evidencia

### Problema

La construcción inicial se ejecutó dentro de una sesión interactiva. Eso es útil para diseñar y depurar, pero no deja un punto de entrada reproducible para una corrida posterior vía API.

### Alternativa descartada

Depender exclusivamente de una conversación interactiva y reconstruir manualmente cada llamada al modelo.

### Decisión

Agregar `runner.py` como capa reproducible para:

- leer el System Prompt y User Prompt versionados;
- recibir un `corrida.json` o resumen estructurado ya calculado;
- realizar la llamada narrativa al modelo;
- capturar telemetría nativa de esa llamada;
- guardar un log transaccional separado.

La lógica de horas y restricciones documentada en el agente no se reemplaza por texto libre del modelo.

### Motivo

Permite separar datos observados de estimaciones, reproducir la etapa de inferencia y dar evidencia verificable para D1.

### Métrica diferencial

- Antes: auditoría 05/09/2026 — D1 = 9/10; faltaba backoff formal y guards.
- Después de este cambio: el código contiene backoff + jitter y límites duros. El nuevo puntaje y la telemetría real se completan únicamente después de una nueva corrida/auditoría.

### Commit

`[COMPLETAR CON SHA REAL DESPUÉS DE HACER EL COMMIT]`

---

## D-23 — Trazabilidad de decisiones con hash de Git

### Problema

Las decisiones documentaban el qué y el porqué, pero no el commit exacto donde se implementaba cada cambio.

### Alternativa descartada

Inventar hashes retroactivos para D-01 a D-21.

### Decisión

Desde D-22 en adelante, cada decisión nueva debe cerrar con un campo `### Commit` que se completa únicamente con el SHA real una vez creado el commit.

### Motivo

Permite vincular decisión → código → evidencia sin falsificar historia del repositorio.

### Métrica diferencial

- Baseline auditado: D2 = 9.5/10 por falta de correlación commit/decisión.
- Objetivo de la próxima auditoría: completar los SHA reales de D-22 a D-24 y registrar una métrica observada antes/después cuando exista una nueva corrida API.
...
## D-24 — Exponential Backoff + Jitter y límites duros

### Problema

La auditoría del 05/09/2026 identificó que el agente no tenía una implementación verificable de reintentos ante HTTP 429/503 ni límites formales de iteraciones/tokens.

### Alternativa descartada

Reintentar inmediatamente o reintentar indefinidamente.

### Decisión

Implementar en `runner.py`:

```text
delay = min(max_delay, base_delay * (2 ** attempt)) + random.uniform(0, 1)
```

Sólo se reintentan errores transitorios:

- HTTP 429;
- HTTP 503;
- timeout/conexión transitoria.
...
se validan como límites duros antes de la ejecución.

### Motivo

Evita thundering herd, controla costo/latencia y cierra específicamente el gap D1 / RES-01 señalado por la auditoría.
...
## D-25 — Análisis económico detectable por la rúbrica

### Problema

La auditoría del 05/09/2026 asignó D4 = 1/10 aunque existía una sección económica previa; el evaluador solicitó explícitamente fórmula, supuestos de volumen, modelo y sensibilidad de Prompt Caching/picos.

### Decisión

Mantener un `COSTOS.md` dedicado y, además, incluir en `README.md` una versión autocontenida con:

- fórmula sin caching;
- fórmula con cached input;
- precios de referencia;
- supuestos de input/output y frecuencia;
- matriz 0%/50%/80%/95% de cache;
- sensibilidad de escala;
- criterio de elegir el modelo más chico que pase todos los tests críticos.

### Motivo

Evita depender de que el evaluador siga un link externo para puntuar D4 y hace auditable el orden de magnitud matemático.
